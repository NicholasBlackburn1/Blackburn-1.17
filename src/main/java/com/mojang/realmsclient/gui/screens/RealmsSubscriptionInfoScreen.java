...
public class RealmsSubscriptionInfoScreen extends RealmsScreen
{
    ...
    private Component daysLeft = f_182537_;
    private Component startDate = f_182537_;
    @Nullable
    ...
    {
        return CommonComponents.m_178396_(SUBSCRIPTION_TITLE, SUBSCRIPTION_START_LABEL, this.startDate, TIME_LEFT_LABEL, this.daysLeft);
    }
    ...
    private void getSubscription(long p_89990_)
    {
        ...
        try
        {
            ...
            this.startDate = m_182538_(subscription.startDate);
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        drawCenteredString(pMatrixStack, this.font, SUBSCRIPTION_TITLE, this.width / 2, 17, 16777215);
        this.font.draw(pMatrixStack, SUBSCRIPTION_START_LABEL, (float)i, (float)row(0), 10526880);
        this.font.draw(pMatrixStack, this.startDate, (float)i, (float)row(1), 16777215);

        if (this.type == Subscription.SubscriptionType.NORMAL)
        {
            this.font.draw(pMatrixStack, TIME_LEFT_LABEL, (float)i, (float)row(3), 10526880);
        }
        else if (this.type == Subscription.SubscriptionType.RECURRING)
        {
            this.font.draw(pMatrixStack, DAYS_LEFT_LABEL, (float)i, (float)row(3), 10526880);
        }

        this.font.draw(pMatrixStack, this.daysLeft, (float)i, (float)row(4), 16777215);

...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen
{
                ...
                while (worldtemplatepaginatedlist != null)
                {
                    ...
                    worldtemplatepaginatedlist = RealmsSelectWorldTemplateScreen.this.minecraft.submit(() ->
                    {
                        if (either.right().isPresent())
                        {
                            ...
                            {
                                RealmsSelectWorldTemplateScreen.this.noTemplatesMessage = TextRenderingUtils.m_90256_(I18n.m_118938_("mco.template.select.failure"));
                            }
                            ...
                            if (worldtemplatepaginatedlist1.templates.isEmpty())
                            {
                                if (RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.isEmpty())
                                {
                                    ...
                                    TextRenderingUtils.LineSegment textrenderingutils$linesegment = TextRenderingUtils.LineSegment.link(I18n.m_118938_("mco.template.select.none.linkTitle"), "https://aka.ms/MinecraftRealmsContentCreator");
                                    RealmsSelectWorldTemplateScreen.this.noTemplatesMessage = TextRenderingUtils.m_90256_(s, textrenderingutils$linesegment);
                                }
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        this.worldTemplateObjectSelectionList.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);

        if (this.noTemplatesMessage != null)
        {
            this.renderMultilineMessage(pMatrixStack, pMouseX, pMouseY, this.noTemplatesMessage);
        }
        ...
        if (this.displayWarning)
        {
            ...
            for (int i1 = 0; i1 < acomponent.length; ++i1)
            {
                ...
                drawCenteredString(pMatrixStack, this.font, component, this.width / 2, row(-1 + i1), j1);
            }
        }
    ...
    class Entry extends ObjectSelectionList.Entry<RealmsSelectWorldTemplateScreen.Entry>
    {
        ...
        {
            this.renderWorldTemplateItem(pMatrixStack, this.template, pLeft, pTop, pMouseX, pMouseY);
        }
        ...
        {
            Component component = CommonComponents.m_178396_(new TextComponent(this.template.name), new TranslatableComponent("mco.template.select.narrate.authors", this.template.author), new TextComponent(this.template.recommendedPlayers), new TranslatableComponent("mco.template.select.narrate.version", this.template.version));
    ...
    class WorldTemplateObjectSelectionList extends RealmsObjectSelectionList<RealmsSelectWorldTemplateScreen.Entry>
    {
        ...
        public boolean mouseClicked(double pMouseX, double p_89798_, int pMouseY)
        {
            if (pMouseY == 0 && p_89798_ >= (double)this.y0 && p_89798_ <= (double)this.y1)
            {
                ...
                {
                    this.selectItem(k);
                    this.itemClicked(j, k, pMouseX, p_89798_, this.width);

                    if (k >= RealmsSelectWorldTemplateScreen.this.worldTemplateObjectSelectionList.getItemCount())
                    {
                        return super.mouseClicked(pMouseX, p_89798_, pMouseY);
                    }
        ...
        public void setSelected(@Nullable RealmsSelectWorldTemplateScreen.Entry pEntry)
        {
            super.setSelected(pEntry);
            RealmsSelectWorldTemplateScreen.this.selectedTemplate = this.children().indexOf(pEntry);
        ...
        {
            RealmsSelectWorldTemplateScreen.this.renderBackground(pMatrixStack);
        }

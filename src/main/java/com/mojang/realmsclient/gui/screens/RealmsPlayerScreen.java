...
public class RealmsPlayerScreen extends RealmsScreen
{
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        {
            this.invitedObjectSelectionList.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
        }
        ...
        {
            this.font.draw(pMatrixStack, (new TextComponent("")).append(INVITED_LABEL).append(" (").append(Integer.toString(this.serverData.players.size())).append(")"), (float)this.column1X, (float)row(0), 10526880);
        }
        else
        {
            this.font.draw(pMatrixStack, INVITED_LABEL, (float)this.column1X, (float)row(0), 10526880);
        }
    ...
    class Entry extends ObjectSelectionList.Entry<RealmsPlayerScreen.Entry>
    {
        ...
        {
            this.renderInvitedItem(pMatrixStack, this.playerInfo, pLeft, pTop, pMouseX, pMouseY);
        }
    ...
    class InvitedObjectSelectionList extends RealmsObjectSelectionList<RealmsPlayerScreen.Entry>
    {
        ...
        public boolean mouseClicked(double pMouseX, double p_89231_, int pMouseY)
        {
            if (pMouseY == 0 && pMouseX < (double)this.getScrollbarPosition() && p_89231_ >= (double)this.y0 && p_89231_ <= (double)this.y1)
            {
                ...
                {
                    this.selectItem(l);
                    this.itemClicked(k, l, pMouseX, p_89231_, this.width);
                }
            ...
            {
                return super.mouseClicked(pMouseX, p_89231_, pMouseY);
            }
        }
        ...
        public void setSelected(@Nullable RealmsPlayerScreen.Entry pEntry)
        {
            super.setSelected(pEntry);
            RealmsPlayerScreen.this.player = this.children().indexOf(pEntry);
            RealmsPlayerScreen.this.updateButtonStates();
        }

        public void renderBackground(PoseStack pMatrixStack)
        {
            RealmsPlayerScreen.this.renderBackground(pMatrixStack);
        }

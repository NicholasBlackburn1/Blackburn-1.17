...
public class RealmsConfigureWorldScreen extends RealmsScreen
{
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        drawCenteredString(pMatrixStack, this.font, WORLD_LIST_TITLE, this.width / 2, row(4), 16777215);
        ...
        else
        {
            ...
            drawCenteredString(pMatrixStack, this.font, s, this.width / 2, 24, j);
            int l = Math.min(this.centerButton(2, 3) + 80 - 11, this.width / 2 + i / 2 + k / 2 + 10);
            this.drawServerStatus(pMatrixStack, l, 7, pMouseX, pMouseY);

            if (this.isMinigame())
            {
                this.font.draw(pMatrixStack, MINIGAME_PREFIX.copy().append(this.serverData.getMinigameName()), (float)(this.leftX + 80 + 20 + 10), (float)row(13), 16777215);
            }

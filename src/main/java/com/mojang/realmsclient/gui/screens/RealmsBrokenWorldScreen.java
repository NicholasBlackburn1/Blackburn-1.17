...
public class RealmsBrokenWorldScreen extends RealmsScreen
{
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        {
            drawCenteredString(pMatrixStack, this.font, this.message[i], this.width / 2, row(-1) + 3 + i * 12, 10526880);
        }
        ...
        if (this.serverData != null)
        {
            for (Entry<Integer, RealmsWorldOptions> entry : this.serverData.slots.entrySet())
            {
                ...
                {
                    this.drawSlotFrame(pMatrixStack, this.getFramePositionX(entry.getKey()), row(1) + 5, pMouseX, pMouseY, this.serverData.activeSlot == entry.getKey() && !this.isMinigame(), entry.getValue().getSlotName(entry.getKey()), entry.getKey(), (entry.getValue()).templateId, (entry.getValue()).templateImage, (entry.getValue()).empty);
                }
                else
                {
                    this.drawSlotFrame(pMatrixStack, this.getFramePositionX(entry.getKey()), row(1) + 5, pMouseX, pMouseY, this.serverData.activeSlot == entry.getKey() && !this.isMinigame(), entry.getValue().getSlotName(entry.getKey()), entry.getKey(), -1L, (String)null, (entry.getValue()).empty);
                }
            }
        }
    }

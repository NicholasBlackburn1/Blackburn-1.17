...
public final class NativeImage implements AutoCloseable
{
    ...
    public static NativeImage read(InputStream pPixelFormat) throws IOException
    {
        return read(NativeImage.Format.RGBA, pPixelFormat);
    }

    public static NativeImage read(@Nullable NativeImage.Format pPixelFormat, InputStream pInputStream) throws IOException
    {
        ...
        try
        {
            ...
            nativeimage = read(pPixelFormat, bytebuffer);
        }
        finally
        {
    ...
    public static NativeImage read(ByteBuffer pPixelFormat) throws IOException
    {
        return read(NativeImage.Format.RGBA, pPixelFormat);
    }

    public static NativeImage read(@Nullable NativeImage.Format pPixelFormat, ByteBuffer pInputStream) throws IOException
    {
        if (pPixelFormat != null && !pPixelFormat.supportedByStb())
        {
            throw new UnsupportedOperationException("Don't know how to read format " + pPixelFormat);
        }
        else if (MemoryUtil.memAddress(pInputStream) == 0L)
        {
        ...
        else
        {
            ...
            try
            {
                ...
                ByteBuffer bytebuffer = STBImage.stbi_load_from_memory(pInputStream, intbuffer, intbuffer1, intbuffer2, pPixelFormat == null ? 0 : pPixelFormat.components);
                ...
                nativeimage = new NativeImage(pPixelFormat == null ? NativeImage.Format.getStbFormat(intbuffer2.get(0)) : pPixelFormat, intbuffer.get(0), intbuffer1.get(0), true, MemoryUtil.memAddress(bytebuffer));
    ...
    public void setPixelRGBA(int pX, int pY, int pValue)
    {
        if (this.format != NativeImage.Format.RGBA)
        {
            throw new IllegalArgumentException(String.format("getPixelRGBA only works on RGBA images; have %s", this.format));
        ...
        else
        {
            this.checkAllocated();
            long i = ((long)pX + (long)pY * (long)this.width) * 4L;
            MemoryUtil.memPutInt(this.pixels + i, pValue);
        }
    }
    ...
    public void upload(int pLevel, int pXOffset, int pYOffset, boolean pUnpackSkipPixels)
    {
        this.upload(pLevel, pXOffset, pYOffset, 0, 0, this.width, this.height, false, pUnpackSkipPixels);
    }
    ...
    public void upload(int pLevel, int pXOffset, int pYOffset, int pUnpackSkipPixels, int pUnpackSkipRows, int pWidth, int pHeight, boolean pMipmap, boolean pAutoClose, boolean p_85023_, boolean p_85024_)
    {
        if (!RenderSystem.isOnRenderThreadOrInit())
        {
            RenderSystem.recordRenderCall(() ->
            {
                this._upload(pLevel, pXOffset, pYOffset, pUnpackSkipPixels, pUnpackSkipRows, pWidth, pHeight, pMipmap, pAutoClose, p_85023_, p_85024_);
            });
        }
        else
        {
    ...
    {
        this.writeToFile(FileSystems.getDefault().getPath(pFile));
    }
    ...
    public void writeToFile(Path pFile) throws IOException
    {
        ...
        {
            this.checkAllocated();
            WritableByteChannel writablebytechannel = Files.newByteChannel(pFile, OPEN_OPTIONS);
    ...
    public byte[] asByteArray() throws IOException
    {
        ...
        catch (Throwable throwable31)
        {
            ...
            {
                throwable31.addSuppressed(throwable);
            }
    ...
    public void copyFrom(NativeImage pFrom)
    {
        if (pFrom.format() != this.format)
        {
        ...
        else
        {
            ...
            this.checkAllocated();
            pFrom.checkAllocated();

            if (this.width == pFrom.width)
            {
                MemoryUtil.memCopy(pFrom.pixels, this.pixels, Math.min(this.size, pFrom.size));
            }
            else
            {
                int j = Math.min(this.getWidth(), pFrom.getWidth());
                int k = Math.min(this.getHeight(), pFrom.getHeight());

                for (int l = 0; l < k; ++l)
                {
                    int i1 = l * pFrom.getWidth() * i;
                    int j1 = l * this.getWidth() * i;
                    MemoryUtil.memCopy(pFrom.pixels + (long)i1, this.pixels + (long)j1, (long)j * (long)i);
                }
            }
        }
    }
    ...
    public static int getA(int pCol)
    {
        return pCol >> 24 & 255;
    }

    public static int getR(int pCol)
    {
        return pCol >> 0 & 255;
    }

    public static int getG(int pCol)
    {
        return pCol >> 8 & 255;
    }

    public static int getB(int pCol)
    {
        return pCol >> 16 & 255;
    }
    ...
    public void fillRGBA(int rgba)
    {
        ...
        {
            this.checkAllocated();
            MemoryUtil.memSet(this.pixels, rgba, this.size);
        }
    }

...
public class InputConstants
{
    ...
    public static InputConstants.Key getKey(String pKeyCode)
    {
        if (InputConstants.Key.NAME_MAP.containsKey(pKeyCode))
        {
            return InputConstants.Key.NAME_MAP.get(pKeyCode);
        }
        else
        {
            for (InputConstants.Type inputconstants$type : InputConstants.Type.values())
            {
                if (pKeyCode.startsWith(inputconstants$type.defaultPrefix))
                {
                    String s = pKeyCode.substring(inputconstants$type.defaultPrefix.length() + 1);
                    return inputconstants$type.getOrCreate(Integer.parseInt(s));
                }
            }

            throw new IllegalArgumentException("Unknown key name: " + pKeyCode);
        }
    }

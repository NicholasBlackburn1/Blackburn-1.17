...
public class Channel
{
    ...
    public void attachBufferStream(AudioStream pAudioStream)
    {
        this.stream = pAudioStream;
        AudioFormat audioformat = pAudioStream.getFormat();
        this.streamingBufferSize = calculateBufferSize(audioformat, 1);
        this.pumpBuffers(4);
    }

    private static int calculateBufferSize(AudioFormat pAudioFormat, int pSampleAmount)
    {
        return (int)((float)(pSampleAmount * pAudioFormat.getSampleSizeInBits()) / 8.0F * (float)pAudioFormat.getChannels() * pAudioFormat.getSampleRate());
    }

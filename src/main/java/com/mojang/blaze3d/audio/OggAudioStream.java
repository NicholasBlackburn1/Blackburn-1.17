...
public class OggAudioStream implements AudioStream
{
    ...
    private boolean readFrame(OggAudioStream.OutputConcat pOggAudioBuffer) throws IOException
    {
        if (this.handle == 0L)
        {
            return false;
            ...
            label79:
            {
                ...
                label80:
                {
                    try
                    {
                        ...
                        while (true)
                        {
                            int z = STBVorbis.stb_vorbis_decode_frame_pushdata(this.handle, this.buffer, intbuffer, pointerbuffer, intbuffer1);
                            this.buffer.position(this.buffer.position() + z);
                            ...
                            else
                            {
                                ...
                                if (j != 0)
                                {
                                    ...
                                    if (k == 1)
                                    {
                                        this.convertMono(pointerbuffer1.getFloatBuffer(0, j), pOggAudioBuffer);
                                        flag = true;
                                        break label80;
                                    }
                                    ...
                                    this.convertStereo(pointerbuffer1.getFloatBuffer(0, j), pointerbuffer1.getFloatBuffer(1, j), pOggAudioBuffer);
                                    flag = true;
                                    break;
                                }
                            }
    ...
    private void convertMono(FloatBuffer pFloatBuffer, OggAudioStream.OutputConcat pOggAudioBuffer)
    {
        while (pFloatBuffer.hasRemaining())
        {
            pOggAudioBuffer.put(pFloatBuffer.get());
        }
    }

    private void convertStereo(FloatBuffer pSoundChannel1, FloatBuffer pSoundChannel2, OggAudioStream.OutputConcat pOggAudioBuffer)
    {
        while (pSoundChannel1.hasRemaining() && pSoundChannel2.hasRemaining())
        {
            pOggAudioBuffer.put(pSoundChannel1.get());
            pOggAudioBuffer.put(pSoundChannel2.get());
        }
    }
    ...
    static class OutputConcat
    {
        ...
        public void put(float pFloatValue)
        {
            ...
            int i = Mth.clamp((int)(pFloatValue * 32767.5F - 0.5F), -32768, 32767);

...
public class TagParser
{
    ...
    public static CompoundTag parseTag(String pJsonString) throws CommandSyntaxException
    {
        return (new TagParser(new StringReader(pJsonString))).readSingleStruct();
    }
    ...
    private Tag type(String pString)
    {
        try
        {
            if (FLOAT_PATTERN.matcher(pString).matches())
            {
                return FloatTag.valueOf(Float.parseFloat(pString.substring(0, pString.length() - 1)));
            }

            if (BYTE_PATTERN.matcher(pString).matches())
            {
                return ByteTag.valueOf(Byte.parseByte(pString.substring(0, pString.length() - 1)));
            }

            if (LONG_PATTERN.matcher(pString).matches())
            {
                return LongTag.valueOf(Long.parseLong(pString.substring(0, pString.length() - 1)));
            }

            if (SHORT_PATTERN.matcher(pString).matches())
            {
                return ShortTag.valueOf(Short.parseShort(pString.substring(0, pString.length() - 1)));
            }

            if (INT_PATTERN.matcher(pString).matches())
            {
                return IntTag.valueOf(Integer.parseInt(pString));
            }

            if (DOUBLE_PATTERN.matcher(pString).matches())
            {
                return DoubleTag.valueOf(Double.parseDouble(pString.substring(0, pString.length() - 1)));
            }

            if (DOUBLE_PATTERN_NOSUFFIX.matcher(pString).matches())
            {
                return DoubleTag.valueOf(Double.parseDouble(pString));
            }

            if ("true".equalsIgnoreCase(pString))
            {
                return ByteTag.ONE;
            }

            if ("false".equalsIgnoreCase(pString))
            {
                return ByteTag.ZERO;
            }
        }
        ...
        return StringTag.valueOf(pString);
    }
    ...
    private <T extends Number> List<T> readArray(TagType<?> pArrayType, TagType<?> pNumberType) throws CommandSyntaxException
    {

...
public class PotionUtils
{
    ...
    public static List<MobEffectInstance> getAllEffects(Potion pTag, Collection<MobEffectInstance> p_43563_)
    {
        List<MobEffectInstance> list = Lists.newArrayList();
        list.addAll(pTag.getEffects());
    ...
    {
        List<MobEffectInstance> list = Lists.newArrayList();
        list.addAll(getPotion(pTag).getEffects());
        getCustomEffects(pTag, list);
        return list;
    }

    public static List<MobEffectInstance> getCustomEffects(ItemStack pTag)
    {
        return getCustomEffects(pTag.getTag());
    }
    ...
    public static void getCustomEffects(@Nullable CompoundTag pTag, List<MobEffectInstance> p_43570_)
    {
        if (pTag != null && pTag.contains("CustomPotionEffects", 9))
        {
            ListTag listtag = pTag.getList("CustomPotionEffects", 10);
    ...
    public static Potion getPotion(ItemStack pTag)
    {
        return getPotion(pTag.getTag());
    }

    public static Potion getPotion(@Nullable CompoundTag pTag)
    {
        return pTag == null ? Potions.EMPTY : Potion.byName(pTag.getString("Potion"));
    }

    public static ItemStack setPotion(ItemStack pItem, Potion pPotion)
    {
        ResourceLocation resourcelocation = Registry.POTION.getKey(pPotion);

        if (pPotion == Potions.EMPTY)
        {
            pItem.removeTagKey("Potion");
        }
        else
        {
            pItem.getOrCreateTag().putString("Potion", resourcelocation.toString());
        }

        return pItem;
    }

    public static ItemStack setCustomEffects(ItemStack pItem, Collection<MobEffectInstance> pEffects)
    {
        if (pEffects.isEmpty())
        {
            return pItem;
        ...
        {
            CompoundTag compoundtag = pItem.getOrCreateTag();
    ...
    public static void addPotionTooltip(ItemStack pItem, List<Component> pLores, float pDurationFactor)
    {
        List<MobEffectInstance> list = getMobEffects(pItem);
        List<Pair<Attribute, AttributeModifier>> list1 = Lists.newArrayList();

        if (list.isEmpty())
        {
            pLores.add(NO_EFFECT);
        }
        else
        {
            for (MobEffectInstance mobeffectinstance : list)
            {
                ...
                pLores.add(mutablecomponent.withStyle(mobeffect.getCategory().getTooltipFormatting()));
            }
        }

        if (!list1.isEmpty())
        {
            pLores.add(TextComponent.EMPTY);
            pLores.add((new TranslatableComponent("potion.whenDrank")).withStyle(ChatFormatting.DARK_PURPLE));
            ...
            for (Pair<Attribute, AttributeModifier> pair : list1)
            {
                ...
                if (d0 > 0.0D)
                {
                    pLores.add((new TranslatableComponent("attribute.modifier.plus." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslatableComponent(pair.getFirst().getDescriptionId()))).withStyle(ChatFormatting.BLUE));
                }
                else if (d0 < 0.0D)
                {
                    d1 = d1 * -1.0D;
                    pLores.add((new TranslatableComponent("attribute.modifier.take." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslatableComponent(pair.getFirst().getDescriptionId()))).withStyle(ChatFormatting.RED));
                }
            }
        }
    }

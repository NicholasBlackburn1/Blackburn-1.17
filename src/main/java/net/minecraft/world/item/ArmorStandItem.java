...
public class ArmorStandItem extends Item
{
            ...
            if (level.noCollision((Entity)null, aabb, (p_40505_) ->
        {
        ...
        }) && level.getEntities((Entity)null, aabb).isEmpty())
            {
                if (level instanceof ServerLevel)
                {
                    ...
                    level.gameEvent(pContext.getPlayer(), GameEvent.ENTITY_PLACE, armorstand);
                }
    ...
    private void randomizePose(ArmorStand pArmorStand, Random pRand)
    {
        Rotations rotations = pArmorStand.getHeadPose();
        float f = pRand.nextFloat() * 5.0F;
        float f1 = pRand.nextFloat() * 20.0F - 10.0F;
        Rotations rotations1 = new Rotations(rotations.getX() + f, rotations.getY() + f1, rotations.getZ());
        pArmorStand.setHeadPose(rotations1);
        rotations = pArmorStand.getBodyPose();
        f = pRand.nextFloat() * 10.0F - 5.0F;

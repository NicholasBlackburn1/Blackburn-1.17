...
public class McRegionUpgrader
{
    ...
    static boolean convertLevel(LevelStorageSource.LevelStorageAccess pLevelSave, ProgressListener pProgress)
    {
        pProgress.progressStagePercentage(0);
        List<File> list = Lists.newArrayList();
        List<File> list1 = Lists.newArrayList();
        List<File> list2 = Lists.newArrayList();
        File file1 = pLevelSave.getDimensionPath(Level.OVERWORLD);
        File file2 = pLevelSave.getDimensionPath(Level.NETHER);
        File file3 = pLevelSave.getDimensionPath(Level.END);
        ...
        WorldData worlddata = pLevelSave.getDataTag(registryreadops, DataPackConfig.DEFAULT);
        ...
        convertRegions(registryaccess$registryholder, new File(file1, "region"), list, biomesource, 0, i, pProgress);
        convertRegions(registryaccess$registryholder, new File(file2, "region"), list1, new FixedBiomeSource(registry.getOrThrow(Biomes.NETHER_WASTES)), list.size(), i, pProgress);
        convertRegions(registryaccess$registryholder, new File(file3, "region"), list2, new FixedBiomeSource(registry.getOrThrow(Biomes.THE_END)), list.size() + list1.size(), i, pProgress);
        makeMcrLevelDatBackup(pLevelSave);
        pLevelSave.saveDataTag(registryaccess$registryholder, worlddata);
        return true;
    }

    private static void makeMcrLevelDatBackup(LevelStorageSource.LevelStorageAccess pLevelSave)
    {
        File file1 = pLevelSave.getLevelPath(LevelResource.LEVEL_DATA_FILE).toFile();
    ...
    private static void addRegionFiles(File pSaveFolder, Collection<File> pFiles)
    {
        File file1 = new File(pSaveFolder, "region");
        ...
        if (afile != null)
        {
            Collections.addAll(pFiles, afile);
        }
    }
}

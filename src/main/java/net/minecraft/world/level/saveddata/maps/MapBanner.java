...
public class MapBanner
{
    ...
    public static MapBanner load(CompoundTag pNbt)
    {
        BlockPos blockpos = NbtUtils.readBlockPos(pNbt.getCompound("Pos"));
        DyeColor dyecolor = DyeColor.byName(pNbt.getString("Color"), DyeColor.WHITE);
        Component component = pNbt.contains("Name") ? Component.Serializer.fromJson(pNbt.getString("Name")) : null;
        return new MapBanner(blockpos, dyecolor, component);
    }

    @Nullable
    public static MapBanner fromWorld(BlockGetter pReader, BlockPos pPos)
    {

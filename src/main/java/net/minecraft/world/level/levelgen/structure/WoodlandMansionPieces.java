...
public class WoodlandMansionPieces
{
    ...
    static class MansionGrid
    {
        ...
        {
            return (this.floorRooms[p_73748_].get(p_73746_, p_73747_) & 65535) == p_73749_;
        }
        ...
        private boolean cleanEdges(WoodlandMansionPieces.SimpleGrid p_73739_)
        {
            ...
            for (int i = 0; i < p_73739_.height; ++i)
            {
                for (int j = 0; j < p_73739_.width; ++j)
                {
                    ...
                    {
                        int k = 0;
                        k = k + (isHouse(p_73739_, j + 1, i) ? 1 : 0);
                        k = k + (isHouse(p_73739_, j - 1, i) ? 1 : 0);
                        k = k + (isHouse(p_73739_, j, i + 1) ? 1 : 0);
                        k = k + (isHouse(p_73739_, j, i - 1) ? 1 : 0);
                        ...
                        {
                            int l = 0;
                            l = l + (isHouse(p_73739_, j + 1, i + 1) ? 1 : 0);
                            l = l + (isHouse(p_73739_, j - 1, i + 1) ? 1 : 0);
                            l = l + (isHouse(p_73739_, j + 1, i - 1) ? 1 : 0);
                            l = l + (isHouse(p_73739_, j - 1, i - 1) ? 1 : 0);
    ...
    static class MansionPiecePlacer
    {
        ...
        private void createRoof(List<WoodlandMansionPieces.WoodlandMansionPiece> p_73804_, BlockPos p_73805_, Rotation p_73806_, WoodlandMansionPieces.SimpleGrid p_73807_, @Nullable WoodlandMansionPieces.SimpleGrid p_73808_)
        {
            for (int i = 0; i < p_73807_.height; ++i)
            {
                ...
                {
                    BlockPos blockpos = p_73805_.relative(p_73806_.rotate(Direction.SOUTH), 8 + (i - this.startY) * 8);
                    blockpos = blockpos.relative(p_73806_.rotate(Direction.EAST), (j - this.startX) * 8);
                    ...
                    {
                        p_73804_.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureManager, "roof", blockpos.above(3), p_73806_));

                        if (!WoodlandMansionPieces.MansionGrid.isHouse(p_73807_, j + 1, i))
                        {
                            BlockPos blockpos1 = blockpos.relative(p_73806_.rotate(Direction.EAST), 6);
                            p_73804_.add(new WoodlandMansionPieces.WoodlandMansionPiece(this.structureManager, "roof_front", blockpos1, p_73806_));
                        }

                        if (!WoodlandMansionPieces.MansionGrid.isHouse(p_73807_, j - 1, i))
                        {
                            BlockPos blockpos5 = blockpos.relative(p_73806_.rotate(Direction.EAST), 0);
                        ...
                        {
                            BlockPos blockpos6 = blockpos.relative(p_73806_.rotate(Direction.WEST), 1);
                        ...
                        if (!WoodlandMansionPieces.MansionGrid.isHouse(p_73807_, j, i + 1))
                        {
                            BlockPos blockpos7 = blockpos.relative(p_73806_.rotate(Direction.EAST), 6);
    ...
    public static class WoodlandMansionPiece extends TemplateStructurePiece
    {
        ...
        {
            super(StructurePieceType.WOODLAND_MANSION_PIECE, p_163697_, p_163696_, (p_163709_) ->
            {
                return makeSettings(Mirror.valueOf(p_163697_.getString("Mi")), Rotation.valueOf(p_163697_.getString("Rot")));
            });
        }
        ...
        protected void addAdditionalSaveData(ServerLevel p_163700_, CompoundTag p_163701_)
        {
            super.addAdditionalSaveData(p_163700_, p_163701_);
            p_163701_.putString("Rot", this.placeSettings.getRotation().name());
            p_163701_.putString("Mi", this.placeSettings.getMirror().name());
        }
        ...
        protected void handleDataMarker(String pFunction, BlockPos pPos, ServerLevelAccessor pLevel, Random pRand, BoundingBox pSbb)
        {
            if (pFunction.startsWith("Chest"))
            {
                ...
                this.createChest(pLevel, pSbb, pRand, pPos, BuiltInLootTables.WOODLAND_MANSION, blockstate);
            }
            else
            {
                ...
                {
                    case "Mage":
                        abstractillager = EntityType.EVOKER.create(pLevel.getLevel());
                        break;

                    case "Warrior":
                        abstractillager = EntityType.VINDICATOR.create(pLevel.getLevel());
                        break;
                ...
                abstractillager.moveTo(pPos, 0.0F, 0.0F);
                abstractillager.finalizeSpawn(pLevel, pLevel.getCurrentDifficultyAt(abstractillager.blockPosition()), MobSpawnType.STRUCTURE, (SpawnGroupData)null, (CompoundTag)null);
                pLevel.addFreshEntityWithPassengers(abstractillager);

...
public class SpikeFeature extends Feature<SpikeConfiguration>
{
    ...
    public static List<SpikeFeature.EndSpike> getSpikesForLevel(WorldGenLevel pReader)
    {
        Random random = new Random(pReader.getSeed());
    ...
    private void placeSpike(ServerLevelAccessor pLevel, Random pRand, SpikeConfiguration pConfig, SpikeFeature.EndSpike pSpike)
    {
        ...
        {
            if (blockpos.distSqr((double)pSpike.getCenterX(), (double)blockpos.getY(), (double)pSpike.getCenterZ(), false) <= (double)(i * i + 1) && blockpos.getY() < pSpike.getHeight())
            {
        ...
        endcrystal.moveTo((double)pSpike.getCenterX() + 0.5D, (double)(pSpike.getHeight() + 1), (double)pSpike.getCenterZ() + 0.5D, pRand.nextFloat() * 360.0F, 0.0F);

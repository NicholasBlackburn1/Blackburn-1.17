...
public class CampfireBlockEntity extends BlockEntity implements Clearable
{
    ...
    public Optional<CampfireCookingRecipe> getCookableRecipe(ItemStack pItemStack)
    {
        return this.items.stream().noneMatch(ItemStack::isEmpty) ? Optional.empty() : this.level.getRecipeManager().getRecipeFor(RecipeType.CAMPFIRE_COOKING, new SimpleContainer(pItemStack), this.level);
    }

    public boolean placeFood(ItemStack pItemStack, int pCookTime)
    {
        for (int i = 0; i < this.items.size(); ++i)
        {
            ...
            if (itemstack.isEmpty())
            {
                ...
                this.items.set(i, pItemStack.split(1));
                this.markUpdated();
                return true;
            }
        }

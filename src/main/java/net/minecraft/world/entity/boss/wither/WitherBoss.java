...
public class WitherBoss extends Monster implements PowerableMob, RangedAttackMob
{
    ...
    private void performRangedAttack(int pHead, LivingEntity pX)
    {
        this.performRangedAttack(pHead, pX.getX(), pX.getY() + (double)pX.getEyeHeight() * 0.5D, pX.getZ(), pHead == 0 && this.random.nextFloat() < 0.001F);
    }

    private void performRangedAttack(int pHead, double pX, double p_31451_, double pY, boolean p_31453_)
    {
        if (!this.isSilent())
        {
    ...
    public void performRangedAttack(LivingEntity pHead, float pX)
    {
        this.performRangedAttack(0, pHead);
    }

...
public class SetEntityLookTarget extends Behavior<LivingEntity>
{
    ...
    {
        return pOwner.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().stream().anyMatch(this.predicate);
    }
    ...
    protected void start(ServerLevel pLevel, LivingEntity pEntity, long pGameTime)
    {
        ...
        brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).ifPresent((p_23920_) ->
        {
            p_23920_.stream().filter(this.predicate).filter((p_147892_) -> {
                return p_147892_.distanceToSqr(pEntity) <= (double)this.maxDistSqr;

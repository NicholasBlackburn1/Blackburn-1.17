...
public class VillagerHostilesSensor extends NearestVisibleLivingEntitySensor
{
    ...
    private boolean isClose(LivingEntity pLivingEntity, LivingEntity pTarget)
    {
        float f = ACCEPTABLE_DISTANCE_FROM_HOSTILES.get(pTarget.getType());
        return pTarget.distanceToSqr(pLivingEntity) <= (double)(f * f);
    }
    ...
    private boolean isHostile(LivingEntity pLivingEntity)
    {
        return ACCEPTABLE_DISTANCE_FROM_HOSTILES.containsKey(pLivingEntity.getType());
    }
}

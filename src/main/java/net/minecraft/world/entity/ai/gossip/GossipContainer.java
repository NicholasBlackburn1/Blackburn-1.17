...
public class GossipContainer
{
    ...
    private Collection<GossipContainer.GossipEntry> selectGossipsForTransfer(Random pRand, int pGossipAmount)
    {
    ...
    {
        Collection<GossipContainer.GossipEntry> collection = pGossip.selectGossipsForTransfer(pRand, pGossipAmount);
    ...
    public void add(UUID pIdentifier, GossipType pGossipType, int pGossipValue)
    {
        ...
        gossipcontainer$entitygossips.entries.mergeInt(pGossipType, pGossipValue, (p_26173_, p_26174_) ->
        {
            return this.mergeValuesForAddition(pGossipType, p_26173_, p_26174_);
        });
    ...
    public void update(Dynamic<?> pDynamic)
    {
        ...
        {
            return Util.toStream(p_26176_.result());
        }).forEach((p_26162_) ->
        {
    ...
    static class GossipEntry
    {
        ...
        public <T> Dynamic<T> store(DynamicOps<T> pDynamic)
        {
            return new Dynamic<>(pDynamic, pDynamic.createMap(ImmutableMap.of(pDynamic.createString("Target"), SerializableUUID.CODEC.encodeStart(pDynamic, this.target).result().orElseThrow(RuntimeException::new), pDynamic.createString("Type"), pDynamic.createString(this.type.id), pDynamic.createString("Value"), pDynamic.createInt(this.value))));
        }

        public static DataResult<GossipContainer.GossipEntry> load(Dynamic<?> pDynamic)
        {
            return DataResult.unbox(DataResult.instance().group(pDynamic.get("Target").read(SerializableUUID.CODEC), pDynamic.get("Type").asString().map(GossipType::byId), pDynamic.get("Value").asNumber().map(Number::intValue)).apply(DataResult.instance(), GossipContainer.GossipEntry::new));
        }
    }
}

...
public class PiglinSpecificSensor extends Sensor<LivingEntity> {
   ...
   protected void doTick(ServerLevel p_26726_, LivingEntity p_26727_) {
      ...
      for(LivingEntity livingentity : brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).orElse(ImmutableList.of())) {
         if (livingentity instanceof Hoglin) {
            ...
            if (!optional5.isPresent() && p_26727_.canAttack(livingentity) && !PiglinAi.isWearingGold(player)) {
   ...
   private static Optional<BlockPos> findNearestRepellent(ServerLevel p_26735_, LivingEntity p_26736_) {
      return BlockPos.findClosestMatch(p_26736_.blockPosition(), 8, 4, (p_26733_) -> {
         return isValidRepellent(p_26735_, p_26733_);
      });
   }

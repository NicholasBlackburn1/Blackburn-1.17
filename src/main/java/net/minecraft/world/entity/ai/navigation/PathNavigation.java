...
public abstract class PathNavigation {
   ...
   protected abstract boolean canMoveDirectly(Vec3 p_26540_, Vec3 p_26541_, int p_26542_, int p_26543_, int p_26544_);
   ...
   public void recomputePath(BlockPos p_26562_) {
      if (this.path != null && !this.path.isDone() && this.path.getNodeCount() != 0) {
         Node node = this.path.getEndNode();
         Vec3 vec3 = new Vec3(((double)node.x + this.mob.getX()) / 2.0D, ((double)node.y + this.mob.getY()) / 2.0D, ((double)node.z + this.mob.getZ()) / 2.0D);
         if (p_26562_.closerThan(vec3, (double)(this.path.getNodeCount() - this.path.getNextNodeIndex()))) {
            this.recomputePath();
         }

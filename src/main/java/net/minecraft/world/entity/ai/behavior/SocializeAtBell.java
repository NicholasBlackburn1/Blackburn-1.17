...
public class SocializeAtBell extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel p_24170_, LivingEntity p_24171_) {
      ...
      return p_24170_.getRandom().nextInt(100) == 0 && optional.isPresent() && p_24170_.dimension() == optional.get().dimension() && optional.get().pos().closerThan(p_24171_.position(), 4.0D) && brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().stream().anyMatch((p_24189_) -> {
   ...
   protected void start(ServerLevel p_24173_, LivingEntity p_24174_, long p_24175_) {
      ...
      brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).ifPresent((p_24184_) -> {
         p_24184_.stream().filter((p_147971_) -> {
            return EntityType.VILLAGER.equals(p_147971_.getType());
         }).filter((p_147974_) -> {
            return p_147974_.distanceToSqr(p_24174_) <= 32.0D;

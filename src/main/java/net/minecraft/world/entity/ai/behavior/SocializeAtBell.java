...
public class SocializeAtBell extends Behavior<LivingEntity>
{
    ...
    protected boolean checkExtraStartConditions(ServerLevel pLevel, LivingEntity pOwner)
    {
        ...
        return pLevel.getRandom().nextInt(100) == 0 && optional.isPresent() && pLevel.dimension() == optional.get().dimension() && optional.get().pos().closerThan(pOwner.position(), 4.0D) && brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().stream().anyMatch((p_24189_) ->
        {
    ...
    protected void start(ServerLevel pLevel, LivingEntity pEntity, long pGameTime)
    {
        ...
        brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).ifPresent((p_24184_) ->
        {
            p_24184_.stream().filter((p_147971_) -> {
                ...
                return p_147974_.distanceToSqr(pEntity) <= 32.0D;

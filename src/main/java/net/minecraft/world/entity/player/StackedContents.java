...
public class StackedContents
{
    ...
    public void accountStack(ItemStack pStack, int p_36470_)
    {
        if (!pStack.isEmpty())
        {
            int i = getStackingIndex(pStack);
            int j = Math.min(p_36470_, pStack.getCount());
            this.put(i, j);
        }
    }
    ...
    public boolean canCraft(Recipe<?> pRecipe, @Nullable IntList pPackedItemList, int p_36481_)
    {
        return (new StackedContents.RecipePicker(pRecipe)).tryPick(p_36481_, pPackedItemList);
    }

    public int getBiggestCraftableStack(Recipe<?> pRecipe, @Nullable IntList pMaxAmount)
    {
        return this.getBiggestCraftableStack(pRecipe, Integer.MAX_VALUE, pMaxAmount);
    }
    ...
    class RecipePicker
    {
        ...
        public int tryPickAll(int p_36526_, @Nullable IntList pList)
        {
            int i = 0;

...
public final class Shapes {
   ...
   public static double collide(Direction.Axis p_83135_, AABB p_83136_, Stream<VoxelShape> p_83137_, double p_83138_) {
      for(Iterator<VoxelShape> iterator = p_83137_.iterator(); iterator.hasNext(); p_83138_ = iterator.next().collide(p_83135_, p_83136_, p_83138_)) {
   ...
   public static double collide(Direction.Axis p_83128_, AABB p_83129_, LevelReader p_83130_, double p_83131_, CollisionContext p_83132_, Stream<VoxelShape> p_83133_) {
      return collide(p_83129_, p_83130_, p_83131_, p_83132_, AxisCycle.between(p_83128_, Direction.Axis.Z), p_83133_);
   }
   ...
   private static double collide(AABB p_83067_, LevelReader p_83068_, double p_83069_, CollisionContext p_83070_, AxisCycle p_83071_, Stream<VoxelShape> p_83072_) {
      if (!(p_83067_.getXsize() < 1.0E-6D) && !(p_83067_.getYsize() < 1.0E-6D) && !(p_83067_.getZsize() < 1.0E-6D)) {
         if (Math.abs(p_83069_) < 1.0E-7D) {
            ...
            int i = Mth.floor(p_83067_.min(direction$axis) - 1.0E-7D) - 1;
            int j = Mth.floor(p_83067_.max(direction$axis) + 1.0E-7D) + 1;
            int k = Mth.floor(p_83067_.min(direction$axis1) - 1.0E-7D) - 1;
            int l = Mth.floor(p_83067_.max(direction$axis1) + 1.0E-7D) + 1;
            double d0 = p_83067_.min(direction$axis2) - 1.0E-7D;
            double d1 = p_83067_.max(direction$axis2) + 1.0E-7D;
            boolean flag = p_83069_ > 0.0D;
            int i1 = flag ? Mth.floor(p_83067_.max(direction$axis2) - 1.0E-7D) - 1 : Mth.floor(p_83067_.min(direction$axis2) + 1.0E-7D) + 1;
            int j1 = lastC(p_83069_, d0, d1);
            int k1 = flag ? 1 : -1;
            int l1 = i1;
               ...
               for(int i2 = i; i2 <= j; ++i2) {
                  for(int j2 = k; j2 <= l; ++j2) {
                     ...
                     if (k2 < 3) {
                        ...
                        BlockState blockstate = p_83068_.getBlockState(blockpos$mutableblockpos);
                        if ((k2 != 1 || blockstate.hasLargeCollisionShape()) && (k2 != 2 || blockstate.is(Blocks.MOVING_PISTON))) {
                           p_83069_ = blockstate.getCollisionShape(p_83068_, blockpos$mutableblockpos, p_83070_).collide(direction$axis2, p_83067_.move((double)(-blockpos$mutableblockpos.getX()), (double)(-blockpos$mutableblockpos.getY()), (double)(-blockpos$mutableblockpos.getZ())), p_83069_);
                           if (Math.abs(p_83069_) < 1.0E-7D) {
                              return 0.0D;
                           }

                           j1 = lastC(p_83069_, d0, d1);
                        }
                     }
                  }
               }
            ...
            p_83072_.forEach((p_83143_) -> {
               adouble[0] = p_83143_.collide(direction$axis2, p_83067_, adouble[0]);
            });
            return adouble[0];
         }
      } else {
   ...
   private static int lastC(double p_83045_, double p_83046_, double p_83047_) {
      return p_83045_ > 0.0D ? Mth.floor(p_83047_ + p_83045_) + 1 : Mth.floor(p_83046_ + p_83045_) - 1;
   }

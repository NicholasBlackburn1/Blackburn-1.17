...
public class HashCache
{
    ...
    @Nullable
    public String getHash(Path pFile)
    {
        return this.oldCache.get(pFile);
    }

    public void putNew(Path pFile, String pHash)
    {
        this.newCache.put(pFile, pHash);

        if (Objects.equals(this.oldCache.remove(pFile), pHash))
        {
            ++this.hits;
        }
    }

    public boolean had(Path pFile)
    {
        return this.oldCache.containsKey(pFile);
    }

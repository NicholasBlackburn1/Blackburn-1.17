...
public class PlayerPredicate
{
    ...
    private static PlayerPredicate.AdvancementPredicate advancementPredicateFromJson(JsonElement pElement)
    {
        if (pElement.isJsonPrimitive())
        {
            boolean flag = pElement.getAsBoolean();
        ...
        else
        {
            ...
            JsonObject jsonobject = GsonHelper.convertToJsonObject(pElement, "criterion data");
    ...
    public static PlayerPredicate fromJson(@Nullable JsonElement pElement)
    {
        if (pElement != null && !pElement.isJsonNull())
        {
            JsonObject jsonobject = GsonHelper.convertToJsonObject(pElement, "player");
    ...
    private static <T> Stat<T> getStat(StatType<T> pType, ResourceLocation pIdentifier)
    {
        Registry<T> registry = pType.getRegistry();
        T t = registry.get(pIdentifier);

        if (t == null)
        {
            throw new JsonParseException("Unknown object " + pIdentifier + " for stat type " + Registry.STAT_TYPE.getKey(pType));
        }
        else
        {
            return pType.get(t);

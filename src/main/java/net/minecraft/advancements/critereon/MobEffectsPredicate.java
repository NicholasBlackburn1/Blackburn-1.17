...
public class MobEffectsPredicate
{
    ...
    public MobEffectsPredicate and(MobEffect pEffect, MobEffectsPredicate.MobEffectInstancePredicate p_154979_)
    {
        this.effects.put(pEffect, p_154979_);
        return this;
    }

    public boolean matches(Entity pPotions)
    {
        if (this == ANY)
        {
            return true;
        ...
        {
            return pPotions instanceof LivingEntity ? this.matches(((LivingEntity)pPotions).getActiveEffectsMap()) : false;
        }
    }

    public boolean matches(LivingEntity pPotions)
    {
        return this == ANY ? true : this.matches(pPotions.getActiveEffectsMap());
    }

    public boolean matches(Map<MobEffect, MobEffectInstance> pPotions)
    {
        if (this == ANY)
        {
            return true;
        ...
        else
        {
            ...
            {
                MobEffectInstance mobeffectinstance = pPotions.get(entry.getKey());
    ...
    public static MobEffectsPredicate fromJson(@Nullable JsonElement pElement)
    {
        ...
        {
            JsonObject jsonobject = GsonHelper.convertToJsonObject(pElement, "effects");
    ...
    public static class MobEffectInstancePredicate
    {
        ...
        public static MobEffectsPredicate.MobEffectInstancePredicate fromJson(JsonObject pObject)
        {
            MinMaxBounds.Ints minmaxbounds$ints = MinMaxBounds.Ints.fromJson(pObject.get("amplifier"));
            MinMaxBounds.Ints minmaxbounds$ints1 = MinMaxBounds.Ints.fromJson(pObject.get("duration"));
            Boolean obool = pObject.has("ambient") ? GsonHelper.getAsBoolean(pObject, "ambient") : null;
            Boolean obool1 = pObject.has("visible") ? GsonHelper.getAsBoolean(pObject, "visible") : null;

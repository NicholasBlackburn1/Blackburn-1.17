...
public class ComponentUtils
{
    ...
    public static Component formatList(Collection<String> pCollection)
    {
        return formatAndSortList(pCollection, (p_130742_) ->
        {
            return (new TextComponent(p_130742_)).withStyle(ChatFormatting.GREEN);
        });
    }

    public static <T extends Comparable<T>> Component formatAndSortList(Collection<T> pCollection, Function<T, Component> pToTextComponent)
    {
        if (pCollection.isEmpty())
        {
            return TextComponent.EMPTY;
        }
        else if (pCollection.size() == 1)
        {
            return pToTextComponent.apply(pCollection.iterator().next());
        }
        else
        {
            List<T> list = Lists.newArrayList(pCollection);
            list.sort(Comparable::compareTo);
            return formatList(list, pToTextComponent);
        }
    }

    public static <T> Component formatList(Collection <? extends T > pCollection, Function<T, Component> p_178442_)
    {
        return formatList(pCollection, DEFAULT_SEPARATOR, p_178442_);
    }

    public static <T> MutableComponent formatList(Collection <? extends T > pCollection, Optional <? extends Component > p_178431_, Function<T, Component> p_178432_)
    {
        return formatList(pCollection, DataFixUtils.orElse(p_178431_, DEFAULT_SEPARATOR), p_178432_);
    }

    public static Component formatList(Collection <? extends Component > pCollection, Component p_178435_)
    {
        return formatList(pCollection, p_178435_, Function.identity());
    }

    public static <T> MutableComponent formatList(Collection <? extends T > pCollection, Component p_178438_, Function<T, Component> p_178439_)
    {
        if (pCollection.isEmpty())
        {
        ...
        {
            return p_178439_.apply(pCollection.iterator().next()).copy();
        }
        else
        {

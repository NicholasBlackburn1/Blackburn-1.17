...
default String getString(int pMaxLen)
    {
        StringBuilder stringbuilder = new StringBuilder();
        this.visit((p_130673_) ->
        {
            int i = pMaxLen - stringbuilder.length();
...
default <T> Optional<T> visit(FormattedText.StyledContentConsumer<T> pAcceptor, Style p_130680_)
    {
...
default List<Component> toFlatList(Style p_178406_)
    ...
    public static class Serializer implements JsonDeserializer<MutableComponent>, JsonSerializer<Component>
    {
        ...
        public static MutableComponent fromJson(com.mojang.brigadier.StringReader pJson)
        {
            try
            {
                JsonReader jsonreader = new JsonReader(new StringReader(pJson.getRemaining()));
                jsonreader.setLenient(false);
                MutableComponent mutablecomponent = GSON.getAdapter(MutableComponent.class).read(jsonreader);
                pJson.setCursor(pJson.getCursor() + getPos(jsonreader));
                return mutablecomponent;
            }

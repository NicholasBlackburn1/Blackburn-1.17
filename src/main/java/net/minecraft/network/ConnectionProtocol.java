...
public enum ConnectionProtocol {
   ...
   static {
      for(ConnectionProtocol connectionprotocol : values()) {
         ...
         connectionprotocol.flows.forEach((p_129590_, p_129591_) -> {
            p_129591_.getAllPackets().forEach((p_178320_) -> {
               if (PROTOCOL_BY_PACKET.containsKey(p_178320_) && PROTOCOL_BY_PACKET.get(p_178320_) != connectionprotocol) {
                  throw new IllegalStateException("Packet " + p_178320_ + " is already assigned to protocol " + PROTOCOL_BY_PACKET.get(p_178320_) + " - can't reassign to " + connectionprotocol);
               } else {
                  PROTOCOL_BY_PACKET.put(p_178320_, connectionprotocol);
               }
            });
         });
      }
   ...
   static class PacketSet<T extends PacketListener> {
      private final Object2IntMap<Class<? extends Packet<T>>> classToId = Util.make(new Object2IntOpenHashMap<>(), (p_129613_) -> {

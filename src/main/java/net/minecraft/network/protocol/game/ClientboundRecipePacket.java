...
public class ClientboundRecipePacket implements Packet<ClientGamePacketListener>
{
    ...
    {
        pBuf.writeEnum(this.state);
        this.bookSettings.write(pBuf);
        pBuf.writeCollection(this.recipes, FriendlyByteBuf::writeResourceLocation);

        if (this.state == ClientboundRecipePacket.State.INIT)
        {
            pBuf.writeCollection(this.toHighlight, FriendlyByteBuf::writeResourceLocation);
        }
    }

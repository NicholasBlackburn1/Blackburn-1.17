...
public class ServerboundClientInformationPacket implements Packet<ServerGamePacketListener>
{
    ...
    {
        pBuf.writeUtf(this.language);
        pBuf.writeByte(this.viewDistance);
        pBuf.writeEnum(this.chatVisibility);
        pBuf.writeBoolean(this.chatColors);
        pBuf.writeByte(this.modelCustomisation);
        pBuf.writeEnum(this.mainHand);
        pBuf.writeBoolean(this.textFilteringEnabled);
    }

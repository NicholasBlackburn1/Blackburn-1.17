...
public class ServerboundSetStructureBlockPacket implements Packet<ServerGamePacketListener>
{
    ...
    public void write(FriendlyByteBuf pBuf)
    {
        ...
        pBuf.writeEnum(this.updateType);
        pBuf.writeEnum(this.mode);
        pBuf.writeUtf(this.name);
        pBuf.writeByte(this.offset.getX());
        pBuf.writeByte(this.offset.getY());
        pBuf.writeByte(this.offset.getZ());
        pBuf.writeByte(this.size.getX());
        pBuf.writeByte(this.size.getY());
        pBuf.writeByte(this.size.getZ());
        pBuf.writeEnum(this.mirror);
        pBuf.writeEnum(this.rotation);
        pBuf.writeUtf(this.data);
        pBuf.writeFloat(this.integrity);
        pBuf.writeVarLong(this.seed);
        int i = 0;

...
public class ClientboundMapItemDataPacket implements Packet<ClientGamePacketListener>
{
    ...
    {
        pBuf.writeVarInt(this.mapId);
        pBuf.writeByte(this.scale);
        pBuf.writeBoolean(this.locked);

        if (this.decorations != null)
        {
            pBuf.writeBoolean(true);
            pBuf.writeCollection(this.decorations, (p_178978_, p_178979_) ->
            {
        ...
        {
            pBuf.writeByte(this.colorPatch.width);
            pBuf.writeByte(this.colorPatch.height);
            pBuf.writeByte(this.colorPatch.startX);
            pBuf.writeByte(this.colorPatch.startY);
            pBuf.m_130087_(this.colorPatch.mapColors);
        }
        else
        {
            pBuf.writeByte(0);

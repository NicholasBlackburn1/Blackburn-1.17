...
public class ClientboundLoginPacket implements Packet<ClientGamePacketListener>
{
    ...
    {
        pBuf.writeInt(this.playerId);
        pBuf.writeBoolean(this.hardcore);
        pBuf.writeByte(this.gameType.getId());
        pBuf.writeByte(GameType.getNullableId(this.previousGameType));
        pBuf.writeCollection(this.levels, (p_178962_, p_178963_) ->
        {
            p_178962_.writeResourceLocation(p_178963_.location());
        });
        pBuf.writeWithCodec(RegistryAccess.RegistryHolder.NETWORK_CODEC, this.registryHolder);
        pBuf.writeWithCodec(DimensionType.CODEC, () ->
        {
            return this.dimensionType;
        });
        pBuf.writeResourceLocation(this.dimension.location());
        pBuf.writeLong(this.seed);
        pBuf.writeVarInt(this.maxPlayers);
        pBuf.writeVarInt(this.chunkRadius);
        pBuf.writeBoolean(this.reducedDebugInfo);
        pBuf.writeBoolean(this.showDeathScreen);
        pBuf.writeBoolean(this.isDebug);
        pBuf.writeBoolean(this.isFlat);
    }

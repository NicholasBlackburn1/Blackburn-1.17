...
public class ClientboundSectionBlocksUpdatePacket implements Packet<ClientGamePacketListener>
{
    ...
    {
        pBuf.writeLong(this.sectionPos.asLong());
        pBuf.writeBoolean(this.suppressLightUpdates);
        pBuf.writeVarInt(this.positions.length);

        for (int i = 0; i < this.positions.length; ++i)
        {
            pBuf.writeVarLong((long)(Block.getId(this.states[i]) << 12 | this.positions[i]));
        }
    }

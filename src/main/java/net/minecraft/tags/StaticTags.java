...
public class StaticTags
{
    ...
    public static void resetAll(TagContainer pSupplier)
    {
        HELPERS.forEach((p_13273_) ->
        {
            p_13273_.reset(pSupplier);
        });
    }
    ...
    public static Multimap < ResourceKey <? extends Registry<? >> , ResourceLocation > getAllMissingTags(TagContainer pSupplier)
    {
        Multimap < ResourceKey <? extends Registry<? >> , ResourceLocation > multimap = HashMultimap.create();
        HELPERS.forEach((p_144348_) ->
        {
            multimap.putAll(p_144348_.getKey(), p_144348_.getMissingTags(pSupplier));
        });
        return multimap;
    }

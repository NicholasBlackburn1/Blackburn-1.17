...
public class DeOpCommands
{
    ...
    public static void register(CommandDispatcher<CommandSourceStack> pDispatcher)
    {
        ...
        {
            return SharedSuggestionProvider.m_82967_(p_136893_.getSource().getServer().getPlayerList().getOpNames(), p_136894_);
        }).executes((p_136891_) ->
        {
    ...
    private static int deopPlayers(CommandSourceStack pSource, Collection<GameProfile> pPlayers) throws CommandSyntaxException
    {
        ...
        for (GameProfile gameprofile : pPlayers)
        {
            if (playerlist.isOp(gameprofile))
            {
                ...
                ++i;
                pSource.sendSuccess(new TranslatableComponent("commands.deop.success", pPlayers.iterator().next().getName()), true);
            }
        }

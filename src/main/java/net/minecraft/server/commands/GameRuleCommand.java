...
public class GameRuleCommand
{
    public static void register(CommandDispatcher<CommandSourceStack> pDispatcher)
    {
        ...
        GameRules.visitGameRuleTypes(new GameRules.GameRuleTypeVisitor()
        {
            ...
            {
                literalargumentbuilder.then(Commands.literal(pKey.getId()).executes((p_137771_) ->
                {
                    return GameRuleCommand.queryRule(p_137771_.getSource(), pKey);
                }).then(pType.createArgument("value").executes((p_137768_) ->
                {
                    return GameRuleCommand.setRule(p_137768_, pKey);
                })));
            }
        });

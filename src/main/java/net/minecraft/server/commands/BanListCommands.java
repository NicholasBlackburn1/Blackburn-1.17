...
public class BanListCommands
{
    ...
    private static int showList(CommandSourceStack pSource, Collection <? extends BanListEntry<? >> pBannedPlayerList)
    {
        ...
        {
            pSource.sendSuccess(new TranslatableComponent("commands.banlist.none"), false);
        }
        else
        {
            pSource.sendSuccess(new TranslatableComponent("commands.banlist.list", pBannedPlayerList.size()), false);

            for (BanListEntry<?> banlistentry : pBannedPlayerList)
            {
                pSource.sendSuccess(new TranslatableComponent("commands.banlist.entry", banlistentry.getDisplayName(), banlistentry.getSource(), banlistentry.getReason()), false);
            }
        }

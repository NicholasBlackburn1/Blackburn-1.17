...
public class SetSpawnCommand
{
    ...
    private static int setSpawn(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, BlockPos pPos, float p_138653_)
    {
        ResourceKey<Level> resourcekey = pSource.getLevel().dimension();

        for (ServerPlayer serverplayer : pTargets)
        {
            serverplayer.setRespawnPosition(resourcekey, pPos, p_138653_, true, false);
        }
        ...
        if (pTargets.size() == 1)
        {
            pSource.sendSuccess(new TranslatableComponent("commands.spawnpoint.success.single", pPos.getX(), pPos.getY(), pPos.getZ(), p_138653_, s, pTargets.iterator().next().getDisplayName()), true);
        }
        else
        {
            pSource.sendSuccess(new TranslatableComponent("commands.spawnpoint.success.multiple", pPos.getX(), pPos.getY(), pPos.getZ(), p_138653_, s, pTargets.size()), true);
        }

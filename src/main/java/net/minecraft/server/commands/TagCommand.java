...
public class TagCommand
{
    ...
    private static int listTags(CommandSourceStack pSource, Collection <? extends Entity > pEntities)
    {
        ...
        if (pEntities.size() == 1)
        {
            ...
            if (set.isEmpty())
            {
                pSource.sendSuccess(new TranslatableComponent("commands.tag.list.single.empty", entity1.getDisplayName()), false);
            }
            else
            {
                pSource.sendSuccess(new TranslatableComponent("commands.tag.list.single.success", entity1.getDisplayName(), set.size(), ComponentUtils.formatList(set)), false);
            }
        }
        else if (set.isEmpty())
        {

...
public class SimpleResource implements Resource
{
    ...
    @Nullable
    public <T> T getMetadata(MetadataSectionSerializer<T> pSerializer)
    {
        if (!this.hasMetadata())
        {
            return (T)null;
        ...
        else
        {
            ...
            {
                String s = pSerializer.getMetadataSectionName();
                return (T)(this.metadata.has(s) ? pSerializer.fromJson(GsonHelper.getAsJsonObject(this.metadata, s)) : null);
            }
        }
    }

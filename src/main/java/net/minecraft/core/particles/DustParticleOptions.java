...
public class DustParticleOptions extends DustParticleOptionsBase
{
    ...
    {
        public DustParticleOptions fromCommand(ParticleType<DustParticleOptions> pParticleType, StringReader pReader) throws CommandSyntaxException
        {
            Vector3f vector3f = DustParticleOptionsBase.readVector3f(pReader);
            pReader.expect(' ');
            float f = pReader.readFloat();
            return new DustParticleOptions(vector3f, f);
        }
        public DustParticleOptions fromNetwork(ParticleType<DustParticleOptions> pParticleType, FriendlyByteBuf pBuffer)
        {
            return new DustParticleOptions(DustParticleOptionsBase.readVector3f(pBuffer), pBuffer.readFloat());
        }
    };

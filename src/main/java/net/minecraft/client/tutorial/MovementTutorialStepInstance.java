...
public class MovementTutorialStepInstance implements TutorialStepInstance
{
    ...
    {
        if (pInput.up || pInput.down || pInput.left || pInput.right || pInput.jumping)
        {
            this.moved = true;
        }
    }

    public void onMouse(double pVelocityX, double p_120526_)
    {
        if (Math.abs(pVelocityX) > 0.01D || Math.abs(p_120526_) > 0.01D)
        {
            this.turned = true;
        }
    }

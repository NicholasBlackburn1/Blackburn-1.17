...
public class HeightMapRenderer implements DebugRenderer.SimpleDebugRenderer
{
    ...
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, double pCamX, double p_113579_, double pCamY)
    {
        ...
        BlockPos blockpos = new BlockPos(pCamX, 0.0D, pCamY);
                ...
                for (Entry<Heightmap.Types, Heightmap> entry : chunkaccess.getHeightmaps())
                {
                    ...
                    for (int k = 0; k < 16; ++k)
                    {
                        for (int l = 0; l < 16; ++l)
                        {
                            ...
                            LevelRenderer.addChainedFilledBoxVertices(bufferbuilder, (double)((float)i1 + 0.25F) - pCamX, (double)f, (double)((float)j1 + 0.25F) - pCamY, (double)((float)i1 + 0.75F) - pCamX, (double)(f + 0.09375F), (double)((float)j1 + 0.75F) - pCamY, vector3f.x(), vector3f.y(), vector3f.z(), 1.0F);
                        }
                    }
                }
            }

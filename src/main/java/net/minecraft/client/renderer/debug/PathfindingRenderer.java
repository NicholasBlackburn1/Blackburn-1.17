...
public class PathfindingRenderer implements DebugRenderer.SimpleDebugRenderer
{
    ...
    public void addPath(int pEid, Path pPath, float pDistance)
    {
        this.pathMap.put(pEid, pPath);
        this.creationMap.put(pEid, Util.getMillis());
        this.pathMaxDist.put(pEid, pDistance);
    }

    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, double pCamX, double p_113632_, double pCamY)
    {
        if (!this.pathMap.isEmpty())
        {
            ...
            for (Integer integer : this.pathMap.keySet())
            {
                ...
                renderPath(path, f, true, true, pCamX, p_113632_, pCamY);
            }

...
public class BatModel extends HierarchicalModel<Bat>
{
    ...
    {
        this.root = p_170427_;
        this.head = p_170427_.getChilds("head");
        this.body = p_170427_.getChilds("body");
        this.rightWing = this.body.getChilds("right_wing");
        this.rightWingTip = this.rightWing.getChilds("right_wing_tip");
        this.leftWing = this.body.getChilds("left_wing");
    ...
    public void setupAnim(Bat pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch)
    {
        if (pEntity.isResting())
        {
            ...
            this.rightWing.yRot = -((float)Math.PI * 2F / 5F);

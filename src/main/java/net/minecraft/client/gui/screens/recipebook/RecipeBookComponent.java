...
public class RecipeBookComponent extends GuiComponent implements Widget, GuiEventListener, NarratableEntry, RecipeShownListener, PlaceRecipe<Ingredient>
{
    ...
    public void init(int pWidth, int pHeight, Minecraft pMinecraft, boolean pWidthTooNarrow, RecipeBookMenu<?> pContainer)
    {
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        if (this.isVisible())
        {
            ...
            pMatrixStack.translate(0.0D, 0.0D, 100.0D);
            ...
            for (RecipeBookTabButton recipebooktabbutton : this.tabButtons)
            {
                recipebooktabbutton.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
            }
    ...
    public boolean mouseClicked(double pMouseX, double p_100295_, int pMouseY)
    {
        if (this.isVisible() && !this.minecraft.player.isSpectator())
        {
            if (this.recipeBookPage.mouseClicked(pMouseX, p_100295_, pMouseY, (this.width - 147) / 2 - this.xOffset, (this.height - 166) / 2, 147, 166))
            {
            ...
            else if (this.searchBox.mouseClicked(pMouseX, p_100295_, pMouseY))
            {
                return true;
            }
            else if (this.filterButton.mouseClicked(pMouseX, p_100295_, pMouseY))
            {
            ...
            else
            {
                for (RecipeBookTabButton recipebooktabbutton : this.tabButtons)
                {
                    if (recipebooktabbutton.mouseClicked(pMouseX, p_100295_, pMouseY))
                    {
    ...
    public boolean isMouseOver(double pMouseX, double p_100354_)
    {
        return false;
    }
    ...
    public void setupGhostRecipe(Recipe<?> p_100316_, List<Slot> p_100317_)
    {
        ...
        this.ghostRecipe.addIngredient(Ingredient.m_43927_(itemstack), (p_100317_.get(0)).x, (p_100317_.get(0)).y);

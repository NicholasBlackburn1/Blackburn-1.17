...
public class EnchantmentNames
{
    ...
    public FormattedText getRandomName(Font pFontRenderer, int pMaxWidth)
    {
        ...
        for (int j = 0; j < i; ++j)
        {
            ...
            stringbuilder.append(Util.m_137545_(this.words, this.random));
        }

        return pFontRenderer.getSplitter().headByWidth((new TextComponent(stringbuilder.toString())).withStyle(ROOT_STYLE), pMaxWidth, Style.EMPTY);
    }

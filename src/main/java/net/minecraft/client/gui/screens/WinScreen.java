...
public class WinScreen extends Screen
{
    ...
    protected void init()
    {
        if (this.lines == null)
        {
            ...
            try
            {
                ...
                JsonArray jsonarray1 = GsonHelper.parseArray(new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8));
                ...
                for (JsonElement jsonelement2 : jsonarray1.getAsJsonArray())
                {
                    ...
                    this.addCreditsLine((new TextComponent(s5)).withStyle(ChatFormatting.YELLOW), true);
                    this.addCreditsLine(SECTION_HEADING, true);
                    this.addEmptyLine();
                    this.addEmptyLine();
                    ...
                    for (JsonElement jsonelement : jsonobject1.getAsJsonArray("titles"))
                    {
                        ...
                        this.addCreditsLine((new TextComponent(s3)).withStyle(ChatFormatting.GRAY), false);

                        for (JsonElement jsonelement1 : jsonarray)
                        {
                            String s4 = jsonelement1.getAsString();
                            this.addCreditsLine((new TextComponent("           ")).append(s4).withStyle(ChatFormatting.WHITE), false);
                        }
    ...
    {
        this.scroll += pPartialTicks * this.scrollSpeed;
        this.renderBg();
        ...
        {
            this.blit(pMatrixStack, p_96890_ + 0, p_96891_, 0, 0, 155, 44);
            this.blit(pMatrixStack, p_96890_ + 155, p_96891_, 0, 45, 155, 44);
        });
        RenderSystem.disableBlend();
        RenderSystem.setShaderTexture(0, EDITION_LOCATION);
        blit(pMatrixStack, i + 88, j + 37, 0.0F, 0.0F, 98, 14, 128, 16);
        int k = j + 100;
        ...
        for (int l = 0; l < this.lines.size(); ++l)
        {
            if (l == this.lines.size() - 1)
            {
                ...
                if (f1 < 0.0F)
                {
                    pMatrixStack.translate(0.0D, (double)(-f1), 0.0D);
                }
            }
            ...
            if ((float)k + f + 12.0F + 8.0F > 0.0F && (float)k + f < (float)this.height)
            {
                ...
                {
                    this.font.drawShadow(pMatrixStack, formattedcharsequence, (float)(i + (274 - this.font.width(formattedcharsequence)) / 2), (float)k, 16777215);
                }
                else
                {
                    this.font.drawShadow(pMatrixStack, formattedcharsequence, (float)i, (float)k, 16777215);
                }
            }

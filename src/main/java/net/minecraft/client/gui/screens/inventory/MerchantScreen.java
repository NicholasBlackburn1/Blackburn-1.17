...
public class MerchantScreen extends AbstractContainerScreen<MerchantMenu>
{
    ...
    protected void renderLabels(PoseStack pMatrixStack, int pX, int pY)
    {
        ...
        if (i > 0 && i <= 5 && this.menu.showProgressBar())
        {
            ...
            this.font.draw(pMatrixStack, component, (float)k, 6.0F, 4210752);
        }
        else
        {
            this.font.draw(pMatrixStack, this.title, (float)(49 + this.imageWidth / 2 - this.font.width(this.title) / 2), 6.0F, 4210752);
        }
        ...
        this.font.draw(pMatrixStack, TRADES_LABEL, (float)(5 - l / 2 + 48), 6.0F, 4210752);
    }
    ...
    private void renderProgressBar(PoseStack p_99153_, int p_99154_, int p_99155_, MerchantOffer p_99156_)
    {
        ...
        if (i < 5)
        {
            blit(p_99153_, p_99154_ + 136, p_99155_ + 16, this.getBlitOffset(), 0.0F, 186.0F, 102, 5, 256, 512);
            ...
            if (j >= k && VillagerData.canLevelUp(i))
            {
                ...
                blit(p_99153_, p_99154_ + 136, p_99155_ + 16, this.getBlitOffset(), 0.0F, 191.0F, i1 + 1, 5, 256, 512);
                int j1 = this.menu.getFutureTraderXp();

                if (j1 > 0)
                {
                    int k1 = Math.min(Mth.floor((float)j1 * f), 100 - i1);
                    blit(p_99153_, p_99154_ + 136 + i1 + 1, p_99155_ + 16 + 1, this.getBlitOffset(), 2.0F, 182.0F, k1, 3, 256, 512);
                }
            }
        }
    }
    ...
    private void renderScroller(PoseStack p_99158_, int p_99159_, int p_99160_, MerchantOffers p_99161_)
    {
        ...
        if (i > 1)
        {
            ...
            blit(p_99158_, p_99159_ + 94, p_99160_ + 18 + i1, this.getBlitOffset(), 0.0F, 199.0F, 6, 27, 256, 512);
        }
        else
        {
            blit(p_99158_, p_99159_ + 94, p_99160_ + 18, this.getBlitOffset(), 6.0F, 199.0F, 6, 27, 256, 512);
        }
    }
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        if (!merchantoffers.isEmpty())
        {
            ...
            {
                this.renderProgressBar(pMatrixStack, i, j, merchantoffer1);
            }
            ...
            {
                if (merchantscreen$tradeofferbutton.isHovered())
                {
                    merchantscreen$tradeofferbutton.renderToolTip(pMatrixStack, pMouseX, pMouseY);
                }
    ...
    private void renderButtonArrows(PoseStack p_99169_, MerchantOffer p_99170_, int p_99171_, int p_99172_)
    {
        ...
        {
            blit(p_99169_, p_99171_ + 5 + 35 + 20, p_99172_ + 3, this.getBlitOffset(), 25.0F, 171.0F, 10, 9, 256, 512);
        }
        else
        {
            blit(p_99169_, p_99171_ + 5 + 35 + 20, p_99172_ + 3, this.getBlitOffset(), 15.0F, 171.0F, 10, 9, 256, 512);
        }
    }
    ...
    private void renderAndDecorateCostA(PoseStack p_99163_, ItemStack p_99164_, ItemStack p_99165_, int p_99166_, int p_99167_)
    {
        ...
        else
        {
            ...
            blit(p_99163_, p_99166_ + 7, p_99167_ + 12, this.getBlitOffset(), 0.0F, 176.0F, 9, 2, 256, 512);
            this.setBlitOffset(this.getBlitOffset() - 300);
        }
    }
    ...
    public boolean mouseScrolled(double pMouseX, double p_99128_, double pMouseY)
    {
        ...
        {
            int j = i - 7;
            this.scrollOff = (int)((double)this.scrollOff - pMouseY);
    ...
    public boolean mouseDragged(double pMouseX, double p_99136_, int pMouseY, double p_99138_, double pButton)
    {
        int i = this.menu.getOffers().size();
        ...
        {
            return super.mouseDragged(pMouseX, p_99136_, pMouseY, p_99138_, pButton);
        }
    }

    public boolean mouseClicked(double pMouseX, double p_99132_, int pMouseY)
    {
        this.isDragging = false;
        int i = (this.width - this.imageWidth) / 2;
        int j = (this.height - this.imageHeight) / 2;

        if (this.canScroll(this.menu.getOffers().size()) && pMouseX > (double)(i + 94) && pMouseX < (double)(i + 94 + 6) && p_99132_ > (double)(j + 18) && p_99132_ <= (double)(j + 18 + 139 + 1))
        {
            this.isDragging = true;
        }

        return super.mouseClicked(pMouseX, p_99132_, pMouseY);
    }
        ...
        public void renderToolTip(PoseStack pMatrixStack, int pMouseX, int pMouseY)
        {
            if (this.isHovered && MerchantScreen.this.menu.getOffers().size() > this.index + MerchantScreen.this.scrollOff)
            {
                if (pMouseX < this.x + 20)
                {
                    ...
                    MerchantScreen.this.renderTooltip(pMatrixStack, itemstack, pMouseX, pMouseY);
                }
                else if (pMouseX < this.x + 50 && pMouseX > this.x + 30)
                {
                    ...
                    if (!itemstack2.isEmpty())
                    {
                        MerchantScreen.this.renderTooltip(pMatrixStack, itemstack2, pMouseX, pMouseY);
                    }
                }
                else if (pMouseX > this.x + 65)
                {
                    ...
                    MerchantScreen.this.renderTooltip(pMatrixStack, itemstack1, pMouseX, pMouseY);
                }
            }
        }
    }

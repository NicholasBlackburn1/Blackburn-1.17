...
public class BeaconScreen extends AbstractContainerScreen<BeaconMenu>
{
    ...
    {
        drawCenteredString(pMatrixStack, this.font, PRIMARY_EFFECT_LABEL, 62, 10, 14737632);
        drawCenteredString(pMatrixStack, this.font, SECONDARY_EFFECT_LABEL, 169, 10, 14737632);

        for (BeaconScreen.BeaconButton beaconscreen$beaconbutton : this.beaconButtons)
        {
            if (beaconscreen$beaconbutton.isShowingTooltip())
            {
                beaconscreen$beaconbutton.renderToolTip(pMatrixStack, pX - this.leftPos, pY - this.topPos);
                break;
            }
        }
    }
    ...
    class BeaconPowerButton extends BeaconScreen.BeaconScreenButton
    {
        ...
        {
            BeaconScreen.this.renderTooltip(pMatrixStack, this.tooltip, pMouseX, pMouseY);
        }

...
public class StatsScreen extends Screen implements StatsUpdateListener
{
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        if (this.isLoading)
        {
            ...
            drawCenteredString(pMatrixStack, this.font, PENDING_TEXT, this.width / 2, this.height / 2, 16777215);
            drawCenteredString(pMatrixStack, this.font, LOADING_SYMBOLS[(int)(Util.getMillis() / 150L % (long)LOADING_SYMBOLS.length)], this.width / 2, this.height / 2 + 9 * 2, 16777215);
        }
        else
        {
            this.getActiveList().render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
    ...
    class GeneralStatisticsList extends ObjectSelectionList<StatsScreen.GeneralStatisticsList.Entry>
    {
        public GeneralStatisticsList(Minecraft p_96995_)
        {
            ...
            {
                return I18n.m_118938_(StatsScreen.getTranslationKey(p_96997_));
            }));
        ...
        {
            StatsScreen.this.renderBackground(pMatrixStack);
        }
        ...
        class Entry extends ObjectSelectionList.Entry<StatsScreen.GeneralStatisticsList.Entry>
        {
            ...
            {
                GuiComponent.drawString(pMatrixStack, StatsScreen.this.font, this.statDisplay, pLeft + 2, pTop + 1, pIndex % 2 == 0 ? 16777215 : 9474192);
                String s = this.getValueText();
                GuiComponent.drawString(pMatrixStack, StatsScreen.this.font, s, pLeft + 2 + 213 - StatsScreen.this.font.width(s), pTop + 1, pIndex % 2 == 0 ? 16777215 : 9474192);
            }
    ...
    class ItemStatisticsList extends ObjectSelectionList<StatsScreen.ItemStatisticsList.ItemRow>
    {
        ...
        protected void renderHeader(PoseStack pMatrixStack, int pX, int pY, Tesselator pTessellator)
        {
            ...
            {
                StatsScreen.this.blitSlotIcon(pMatrixStack, pX + StatsScreen.this.getColumnX(i) - 18, pY + 1, 0, this.headerPressed == i ? 0 : 18);
            }
            ...
            if (this.sortColumn != null)
            {
                ...
                StatsScreen.this.blitSlotIcon(pMatrixStack, pX + k, pY + 1, 18 * j, 0);
            }

            for (int l = 0; l < this.iconOffsets.length; ++l)
            {
                int i1 = this.headerPressed == l ? 1 : 0;
                StatsScreen.this.blitSlotIcon(pMatrixStack, pX + StatsScreen.this.getColumnX(l) - 18 + i1, pY + 1 + i1, 18 * this.iconOffsets[l], 18);
            }
        }
        ...
        protected void renderDecorations(PoseStack pMatrixStack, int pMouseX, int pMouseY)
        {
            if (pMouseY >= this.y0 && pMouseY <= this.y1)
            {
                ...
                if (statsscreen$itemstatisticslist$itemrow != null)
                {
                    ...
                    this.renderMousehoverTooltip(pMatrixStack, this.getString(item), pMouseX, pMouseY);
                }
                else
                {
                    ...
                    this.renderMousehoverTooltip(pMatrixStack, component, pMouseX, pMouseY);
                }
            }
        }
        ...
        class ItemRow extends ObjectSelectionList.Entry<StatsScreen.ItemStatisticsList.ItemRow>
        {
            ...
            public void render(PoseStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks)
            {
                StatsScreen.this.blitSlot(pMatrixStack, pLeft + 40, pTop, this.item);
                ...
                for (int i = 0; i < StatsScreen.this.itemStatsList.blockColumns.size(); ++i)
                {
                    ...
                    this.renderStat(pMatrixStack, stat, pLeft + StatsScreen.this.getColumnX(i), pTop, pIndex % 2 == 0);
                }

                for (int j = 0; j < StatsScreen.this.itemStatsList.itemColumns.size(); ++j)
                {
                    this.renderStat(pMatrixStack, StatsScreen.this.itemStatsList.itemColumns.get(j).get(this.item), pLeft + StatsScreen.this.getColumnX(j + StatsScreen.this.itemStatsList.blockColumns.size()), pTop, pIndex % 2 == 0);
                }
            }
    ...
    class MobsStatisticsList extends ObjectSelectionList<StatsScreen.MobsStatisticsList.MobRow>
    {
        ...
        class MobRow extends ObjectSelectionList.Entry<StatsScreen.MobsStatisticsList.MobRow>
        {
            ...
            public void render(PoseStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks)
            {
                GuiComponent.drawString(pMatrixStack, StatsScreen.this.font, this.mobName, pLeft + 2, pTop + 1, 16777215);
                GuiComponent.drawString(pMatrixStack, StatsScreen.this.font, this.kills, pLeft + 2 + 10, pTop + 1 + 9, this.hasKills ? 9474192 : 6316128);
                GuiComponent.drawString(pMatrixStack, StatsScreen.this.font, this.killedBy, pLeft + 2 + 10, pTop + 1 + 9 * 2, this.wasKilledBy ? 9474192 : 6316128);
            }

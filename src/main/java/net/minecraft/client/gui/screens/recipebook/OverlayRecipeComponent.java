...
public class OverlayRecipeComponent extends GuiComponent implements Widget, GuiEventListener
{
    ...
    public boolean mouseClicked(double pMouseX, double p_100187_, int pMouseY)
    {
        ...
        else
        {
            ...
            {
                if (overlayrecipecomponent$overlayrecipebutton.mouseClicked(pMouseX, p_100187_, pMouseY))
                {
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        if (this.isVisible)
        {
            ...
            pMatrixStack.translate(0.0D, 0.0D, 170.0D);
            ...
            int j1 = 82;
            int k1 = 208;
            this.nineInchSprite(pMatrixStack, j, k, 24, 4, 82, 208);
            RenderSystem.disableBlend();

            for (OverlayRecipeComponent.OverlayRecipeButton overlayrecipecomponent$overlayrecipebutton : this.recipeButtons)
            {
                overlayrecipecomponent$overlayrecipebutton.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
            }

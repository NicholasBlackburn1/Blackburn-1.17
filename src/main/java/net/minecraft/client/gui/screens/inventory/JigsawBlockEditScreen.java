...
public class JigsawBlockEditScreen extends Screen
{
    ...
    protected void init()
    {
        ...
        this.jointButton = this.addRenderableWidget(CycleButton.builder(JigsawBlockEntity.JointType::getTranslatedName).m_168961_(JigsawBlockEntity.JointType.values()).withInitialValue(this.joint).displayOnlyValue().create(this.width / 2 - 152 + i, 150, 300 - i, 20, JOINT_LABEL, (p_169765_, p_169766_) ->
        {
    ...
    public void render(PoseStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        ...
        drawString(pMatrixStack, this.font, POOL_LABEL, this.width / 2 - 153, 10, 10526880);
        this.poolEdit.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
        drawString(pMatrixStack, this.font, NAME_LABEL, this.width / 2 - 153, 45, 10526880);
        this.nameEdit.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
        drawString(pMatrixStack, this.font, TARGET_LABEL, this.width / 2 - 153, 80, 10526880);
        this.targetEdit.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
        drawString(pMatrixStack, this.font, FINAL_STATE_LABEL, this.width / 2 - 153, 115, 10526880);
        this.finalStateEdit.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);

        if (JigsawBlock.getFrontFacing(this.jigsawEntity.getBlockState()).getAxis().isVertical())
        {
            drawString(pMatrixStack, this.font, JOINT_LABEL, this.width / 2 - 153, 156, 16777215);
        }

...
public class PlayerEntry extends ContainerObjectSelectionList.Entry<PlayerEntry>
{
    ...
    public void render(PoseStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks)
    {
        ...
        {
            GuiComponent.fill(pMatrixStack, pLeft, pTop, pLeft + pWidth, pTop + pHeight, BG_FILL);
            l = pTop + (pHeight - 9) / 2;
        }
        else
        {
            GuiComponent.fill(pMatrixStack, pLeft, pTop, pLeft + pWidth, pTop + pHeight, BG_FILL_REMOVED);
            l = pTop + (pHeight - (9 + 9)) / 2;
            this.minecraft.font.draw(pMatrixStack, component, (float)k, (float)(l + 12), PLAYER_STATUS_COLOR);
        }

        RenderSystem.setShaderTexture(0, this.skinGetter.get());
        GuiComponent.blit(pMatrixStack, i, j, 24, 24, 8.0F, 8.0F, 8, 8, 64, 64);
        RenderSystem.enableBlend();
        GuiComponent.blit(pMatrixStack, i, j, 24, 24, 40.0F, 8.0F, 8, 8, 64, 64);
        RenderSystem.disableBlend();
        this.minecraft.font.draw(pMatrixStack, this.playerName, (float)k, (float)l, PLAYERNAME_COLOR);

        if (this.isRemoved)
        {
            GuiComponent.fill(pMatrixStack, i, j, i + 24, j + 24, SKIN_SHADE);
        }
        ...
        if (this.hideButton != null && this.showButton != null)
        {
            ...
            this.hideButton.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);
            this.showButton.x = pLeft + (pWidth - this.showButton.getWidth() - 4);
            this.showButton.y = pTop + (pHeight - this.showButton.getHeight()) / 2;
            this.showButton.render(pMatrixStack, pMouseX, pMouseY, pPartialTicks);

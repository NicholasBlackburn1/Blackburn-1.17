...
public class DynamicLights
{
    ...
    private static final EntityDataAccessor<ItemStack> PARAMETER_ITEM_STACK = (EntityDataAccessor)Reflector.EntityItem_ITEM.getValue();
    ...
    {
        ClientLevel clientlevel = renderGlobal.getWorld();
    ...
    {
        double d0 = getLightLevel(entity.blockPosition());
    ...
    public static int getLightLevel(ItemStack itemStack)
    {
        ...
        else
        {
            ...
            else if (item != Items.BLAZE_ROD && item != Items.BLAZE_POWDER)
            {
                ...
                else if (item == Items.GLOW_ITEM_FRAME)
                {
                    return 8;
                }
                else
    ...
    {
        if (entity == Config.getMinecraft().getCameraEntity() && !Config.isDynamicHandLight())
        {
            return 0;
        }
        else
        {
            ...
            else
            {
                ...
                else if (entity instanceof MagmaCube)
                {
                    ...
                    return (double)magmacube.squish > 0.6D ? 13 : 8;
                }
                else
                {
                    if (entity instanceof Creeper)
                    {
                        ...
                        if ((double)creeper.getSwelling(0.0F) > 0.001D)
                        {
                            return 15;
                        }
                    }
                    ...
                    else if (entity instanceof LivingEntity)
                    {
                        ...
                        ItemStack itemstack3 = livingentity.getMainHandItem();
                        int i = getLightLevel(itemstack3);
                        ItemStack itemstack = livingentity.getOffhandItem();
                        int j = getLightLevel(itemstack);
                        ItemStack itemstack1 = livingentity.getItemBySlot(EquipmentSlot.HEAD);
    ...
    {
        return entityItem.getEntityData().get(PARAMETER_ITEM_STACK);
    }
}

...
public class ConnectedTextures
{
    ...
    private static boolean skipConnectedTexture(BlockGetter blockAccess, BlockState blockState, BlockPos blockPos, BakedQuad quad, RenderEnv renderEnv)
    {
        ...
        if (block instanceof IronBarsBlock)
        {
            ...
            BlockPos blockpos = blockPos.relative(quad.getDirection());
    ...
    private static synchronized BakedQuad getQuadFull(TextureAtlasSprite sprite, BakedQuad quadIn, int tintIndex)
    {
        ...
        else
        {
            ...
            if (i >= 0 && i < spriteQuadFullMaps.length)
            {
                ...
                Direction direction = quadIn.getDirection();
    ...
    private static BakedQuad makeSpriteQuad(BakedQuad quad, TextureAtlasSprite sprite)
    {
        ...
        return new BakedQuad(aint, quad.getTintIndex(), quad.getDirection(), sprite, quad.isShade());
    }
    ...
    private static void fixVertex(int[] data, int vertex, TextureAtlasSprite spriteFrom, TextureAtlasSprite spriteTo)
    {
        ...
        data[j + 4] = Float.floatToRawIntBits(spriteTo.getU(d0));
        data[j + 4 + 1] = Float.floatToRawIntBits(spriteTo.getV(d1));
    }
    ...
    {
        Direction.Axis direction$axis = blockState.getValue(RotatedPillarBlock.AXIS);
    ...
    private static int getConnectedTextureCtmIndex(ConnectedProperties cp, BlockGetter blockAccess, BlockState blockState, BlockPos blockPos, int vertAxis, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        ...
        switch (side)
        {
            ...
            case 2:
                ...
                aboolean[2] = isNeighbour(cp, blockAccess, blockState, blockPos.below(), side, icon, metadata);
                aboolean[3] = isNeighbour(cp, blockAccess, blockState, blockPos.above(), side, icon, metadata);
                ...
                if (cp.innerSeams)
                {
                    ...
                    aboolean[2] = aboolean[2] && !isNeighbour(cp, blockAccess, blockState, blockpos4.below(), side, icon, metadata);
                    aboolean[3] = aboolean[3] && !isNeighbour(cp, blockAccess, blockState, blockpos4.above(), side, icon, metadata);
                }
            ...
            case 3:
                ...
                if (cp.innerSeams)
                {
                    ...
                    aboolean[2] = aboolean[2] && !isNeighbour(cp, blockAccess, blockState, blockpos3.below(), side, icon, metadata);
                    aboolean[3] = aboolean[3] && !isNeighbour(cp, blockAccess, blockState, blockpos3.above(), side, icon, metadata);
                }
            ...
            case 4:
                ...
                if (cp.innerSeams)
                {
                    ...
                    aboolean[2] = aboolean[2] && !isNeighbour(cp, blockAccess, blockState, blockpos2.below(), side, icon, metadata);
                    aboolean[3] = aboolean[3] && !isNeighbour(cp, blockAccess, blockState, blockpos2.above(), side, icon, metadata);
                }
            ...
            case 5:
                ...
                if (cp.innerSeams)
                {
                    ...
                    aboolean[2] = aboolean[2] && !isNeighbour(cp, blockAccess, blockState, blockpos.below(), side, icon, metadata);
                    aboolean[3] = aboolean[3] && !isNeighbour(cp, blockAccess, blockState, blockpos.above(), side, icon, metadata);
                }
        }
        ...
        else
        {
            switch (side)
            {
                case 0:
                    ...
                    if (cp.innerSeams)
                    {
                        BlockPos blockpos11 = blockPos.below();
                ...
                case 1:
                    ...
                    if (cp.innerSeams)
                    {
                        BlockPos blockpos10 = blockPos.above();
                ...
                case 2:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().below(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().below(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().above(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().above(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        BlockPos blockpos9 = blockPos.north();
                        aboolean[0] = aboolean[0] || isNeighbour(cp, blockAccess, blockState, blockpos9.west().below(), side, icon, metadata);
                        aboolean[1] = aboolean[1] || isNeighbour(cp, blockAccess, blockState, blockpos9.east().below(), side, icon, metadata);
                        aboolean[2] = aboolean[2] || isNeighbour(cp, blockAccess, blockState, blockpos9.west().above(), side, icon, metadata);
                        aboolean[3] = aboolean[3] || isNeighbour(cp, blockAccess, blockState, blockpos9.east().above(), side, icon, metadata);
                    }

                    break;

                case 3:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().below(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().below(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().above(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().above(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        BlockPos blockpos8 = blockPos.south();
                        aboolean[0] = aboolean[0] || isNeighbour(cp, blockAccess, blockState, blockpos8.east().below(), side, icon, metadata);
                        aboolean[1] = aboolean[1] || isNeighbour(cp, blockAccess, blockState, blockpos8.west().below(), side, icon, metadata);
                        aboolean[2] = aboolean[2] || isNeighbour(cp, blockAccess, blockState, blockpos8.east().above(), side, icon, metadata);
                        aboolean[3] = aboolean[3] || isNeighbour(cp, blockAccess, blockState, blockpos8.west().above(), side, icon, metadata);
                    }

                    break;

                case 4:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.below().south(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.below().north(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.above().south(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.above().north(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        BlockPos blockpos7 = blockPos.west();
                        aboolean[0] = aboolean[0] || isNeighbour(cp, blockAccess, blockState, blockpos7.below().south(), side, icon, metadata);
                        aboolean[1] = aboolean[1] || isNeighbour(cp, blockAccess, blockState, blockpos7.below().north(), side, icon, metadata);
                        aboolean[2] = aboolean[2] || isNeighbour(cp, blockAccess, blockState, blockpos7.above().south(), side, icon, metadata);
                        aboolean[3] = aboolean[3] || isNeighbour(cp, blockAccess, blockState, blockpos7.above().north(), side, icon, metadata);
                    }

                    break;

                case 5:
                    aboolean[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.below().north(), side, icon, metadata);
                    aboolean[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.below().south(), side, icon, metadata);
                    aboolean[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.above().north(), side, icon, metadata);
                    aboolean[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.above().south(), side, icon, metadata);

                    if (cp.innerSeams)
                    {
                        BlockPos blockpos1 = blockPos.east();
                        aboolean[0] = aboolean[0] || isNeighbour(cp, blockAccess, blockState, blockpos1.below().north(), side, icon, metadata);
                        aboolean[1] = aboolean[1] || isNeighbour(cp, blockAccess, blockState, blockpos1.below().south(), side, icon, metadata);
                        aboolean[2] = aboolean[2] || isNeighbour(cp, blockAccess, blockState, blockpos1.above().north(), side, icon, metadata);
                        aboolean[3] = aboolean[3] || isNeighbour(cp, blockAccess, blockState, blockpos1.above().south(), side, icon, metadata);
                    }
            }
    ...
    private static boolean isNeighbourOverlay(ConnectedProperties cp, BlockGetter worldReader, BlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata)
    {
        ...
        else
        {
            ...
            BlockPos blockpos = blockPos.relative(getFacing(side));
            BlockState blockstate1 = worldReader.getBlockState(blockpos);

            if (blockstate1.isSolidRender(worldReader, blockpos))
            {
                return false;
            }
    ...
    {
        BakedModel bakedmodel = Minecraft.getInstance().getBlockRenderer().getBlockModelShaper().getBlockModel(neighbourState);
        ...
        else
        {
            ...
            List list = bakedmodel.getQuads(neighbourState, direction, RANDOM);
            ...
            else
            {
                ...
                {
                    List list1 = bakedmodel.getQuads(neighbourState, (Direction)null, RANDOM);
        ...
        switch (vertAxis)
        {
            ...
            case 1:
                switch (side)
                {
                    ...
                    case 4:
                        flag = isNeighbour(cp, blockAccess, blockState, blockPos.below(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, blockPos.above(), side, icon, metadata);
                        break label49;

                    case 5:
                        flag = isNeighbour(cp, blockAccess, blockState, blockPos.above(), side, icon, metadata);
                        flag1 = isNeighbour(cp, blockAccess, blockState, blockPos.below(), side, icon, metadata);
    ...
    public static void updateIcons(TextureAtlas textureMap)
    {
        ...
        if (Config.isConnectedTextures())
        {
            ...
            for (int i = apackresources.length - 1; i >= 0; --i)
            {
                ...
                updateIcons(textureMap, packresources);
            }
    ...
    private static void addDefaultLocation(List list, String locBase, String pathSuffix)
    {
        ...
        PackResources packresources = Config.getDefiningResourcePack(resourcelocation);

        if (packresources != null)
        {
            if (packresources.getName().equals("Programmer Art"))
            {
